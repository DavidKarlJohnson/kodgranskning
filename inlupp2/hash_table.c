...
/// @brief Function for hashing a string into a table with a bucket-size of 17
/// @param key The key element (to be converted to string) to hash
/// @return To bucket position to store the key
int hash_func_str(elem_t key)
{
  int counter = 0;
...
bool eq_int(elem_t a, elem_t b) { return a.i == b.i; }
...
entry_t* find_previous_entry_for_key(ioopm_hash_table_t *ht, entry_t *entry, const elem_t key) 
{
